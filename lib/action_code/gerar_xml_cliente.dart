...
Future<String?> gerarXmlCliente(
  ...
  try {
    ...
    // Cabecalho estrito do modelo Suportware
    xml.writeln('<!DOCTYPE suportware>');
    xml.writeln('<root>');
    xml.writeln(' <pckvencli00>');

    // Tratamento para Novo Cadastro vs Edicao
    // Se for 0 ou nulo, o ERP legado interpreta cli00_codigo="0" como inclusao
    final int codCliInt = clienteData.cli00Codigo ?? 0;
    ...
    final String cpfCnpjLimpo = (clienteData.cli00Cpfcnp ?? '')
        .trim()
    ...
    final String tPessoa =
        (clienteData.cli00Pessoa ?? '').trim() == 'J' ? '2' : '1';
    ...
    final String limite =
        (clienteData.cli00Crelim ?? 0.0).toStringAsFixed(2);
    ...
    // Salvando em UTF-8 nativo do Dart sem o parametro redundante de encoding
