...
1. **Separação Rígida entre Gravação Local e Rede:**
...
2. **Integridade de Chaves e Expurgo Seguro:**
   Nunca executar `DELETE FROM pckvendig010` sem cláusula `WHERE ped10_numped = ?`. Operações globais de limpeza devem sempre ser escopadas pelo ID do pedido.

3. **Garantia Defensiva de Tabelas e Colunas:**
   Toda rotina de escrita no SQLite deve invocar `CREATE TABLE IF NOT EXISTS` para as tabelas principais (`pckvendig000`, `pckvendig010`) e secundárias (`ESTFATCVD00`, `FINCAICVD00`, `ESTPRO00`), além de aplicar `ALTER TABLE ... ADD COLUMN` para colunas opcionais, evitando abortos silenciosos de transações.

4. **Tratamento e Rastreamento Explícito de Exceções:**
   Qualquer falha durante a persistência relacional do pedido deve capturar `(e, stack)` e registrar no console:
   ```dart
   print('ERRO GRAVACAO PEDIDO: $e \n $stack');
   ```

5. **Unificação Leitura e Gravação:**
   A leitura no histórico (`listarPedidosHistorico`) e a gravação (`salvarCarrinhoPedido` / `ConcluirVendaService`) utilizam as mesmas tabelas físicas (`pckvendig000`, `pckvendig010`) e mantêm sincronizadas as views de compatibilidade `dig00` e `dig01`.
